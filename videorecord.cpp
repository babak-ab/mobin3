...
void VideoRecord::initialize(const QString& location)
{
    ...
    QString filename = "";
    if (_resolution.width() == 1920)
        filename = "Tv_video" + dt.toString("_yyyy_MM_dd_HH_mm_ss") + "_%d.mp4";
    else
        filename = "Trc_video" + dt.toString("_yyyy_MM_dd_HH_mm_ss") + "_%d.mp4";
    ...
    QString pipestr = QString("appsrc caps=%1 is-live=true name=src block=TRUE format=3 do-timestamp=true ! queue "
                              "! videoconvert ! "
    ...
    qDebug() << "_location: " << _location << location << fileInfo.isWritable() << dir.exists();
